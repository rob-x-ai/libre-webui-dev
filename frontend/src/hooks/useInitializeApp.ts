...
export const useInitializeApp = () => {
  ...
  useEffect(() => {
    console.log('🔄 useInitializeApp: models effect triggered, models.length:', models.length);
    ...
    if (models.length > 0) {
      ...
      if (currentSelected) {
        ...
        if (!availableModelNames.includes(currentSelected)) {
          ...
          console.log('⚠️ Selected model not available, falling back to first model:', models[0].name);
        ...
        console.log('⚠️ No model selected, using first available:', models[0].name);
        setSelectedModel(models[0].name);
      }
    }
