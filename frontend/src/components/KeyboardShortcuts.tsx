...
export const KeyboardShortcutsModal: React.FC<KeyboardShortcutsModalProps> = ({
  ...
  const shortcutGroups = {
    Navigation: shortcuts.filter(s => s.description.includes('sidebar') || s.description.includes('Toggle')),
    Settings: shortcuts.filter(s => s.description.includes('settings') || s.description.includes('dark mode')),
    General: shortcuts.filter(s => !s.description.includes('sidebar') && !s.description.includes('settings') && !s.description.includes('dark mode') && !s.description.includes('Toggle')),
  };
      ...
      <div className="bg-white dark:bg-dark-100 rounded-xl shadow-2xl w-full max-w-md max-h-[80vh] overflow-hidden">
        ...
        <div className="px-6 py-4 bg-gray-50 dark:bg-dark-200 border-t border-gray-200 dark:border-dark-300">
          <p className="text-xs text-gray-500 dark:text-dark-400 text-center">
            Press <kbd className="px-1 py-0.5 text-xs font-mono bg-gray-200 dark:bg-dark-300 rounded">?</kbd> or <kbd className="px-1 py-0.5 text-xs font-mono bg-gray-200 dark:bg-dark-300 rounded">H</kbd> to show this help
          </p>
        </div>
      </div>
    </div>
...
export const KeyboardShortcutsIndicator: React.FC<{
  ...
  return (
    <Button
      ...
      className={cn(
        "fixed bottom-4 right-4 h-10 w-10 p-0 bg-white/80 dark:bg-dark-100/80 backdrop-blur-sm border border-gray-200 dark:border-dark-300 shadow-lg hover:shadow-xl transition-all duration-200",
